...
uses
  ...
  Menus, ActnList, ComCtrls, Buttons, StdCtrls, GVConsts, GVTurtles,
...
type
  ...
  private
    ...
    procedure TurtleState(Sender: TObject; cX, cY, cHeading: Integer;
      cVisible, cDown: Boolean; cColor: TColor); dynamic;
...
begin
  ...
  Memorize(GVTurtle.CoordX); // ancien X
  Memorize(GVTurtle.CoordY); // ancien Y
  ...
  begin
    case MemoInt[Length(MemoInt) - 1] of
      ...
      CT_Circle: begin  // un cercle
        GVTurtle.PenRubber:= GVTurtle.PenDown;
        GVTurtle.Circle(pLength);
        GVTurtle.PenRubber:= False;
      end;
      CT_Square: begin  // un carré
        GVTurtle.PenRubber:= GVTurtle.PenDown;
        GVTurtle.Square(pLength);
        GVTurtle.PenRubber:= False;
      end;
    end;
...
begin
  ...
  fLength := C_Length;
  fState := stRecording;
  GVTurtle := TGVTurtle.Create(pnlTurtle); // dans le panneau prévu
  GVTurtle.Parent := pnlTurtle;  // parent défini pour l'affichage
  GVTurtle.OnChange := @TurtleState;  // gestionnaire de changement
  GVTurtle.OnBeforeChange := @TurtleBeforePaint; // idem avant de dessiner
  GVTurtle.Cursor := crNo; // curseur personnalisé
...
begin
  ...
  Statusbar.Panels[CSB_Turtle].Text := Format(ME_XY,[cX,cY])
  + Format(ME_Heading, [cHeading]) + ME_TVisible + IfThen(cVisible, P_True,
    P_False) + ME_PenDown + IfThen(cDown, P_True, P_False);
...
var
  BitM: TBitmap;
begin
  BitM := TBitmap.Create; // création du bitmap
  try
    // les images de la tortue sont proposées tous les 5 degrés
    ImageListTurtles.GetBitmap(Round(GVTurtle.Heading) div 5, BitM);
    // celle qui correspond est assignée à la surface de la tortue
    GVTurtle.TurtleImg.Assign(BitM);
  finally
    BitM.Free; // libération du bitmap
  end;
end;
...
begin
  ...
  begin
    ...
    ActionRAZExecute(Sender); // réinitialisation
  end;
end;
...
begin
  if GVTurtle.Size < (TurtleMaxSize - 1) then // pas de taille supérieure à 20
  begin
...
begin
  ...
  gvTurtle.PenColor := clAqua; // écriture bleu clair
  GVTurtle.Screen := teGate; // écran clos
